...
class ProofOfAddressV1Test {
  ...
  void givenProofOfAddressV1_whenDeserialized_MustHaveAValidSummary() throws IOException {
    ...
    PredictResponse<ProofOfAddressV1Inference> prediction = objectMapper.readValue(
        new File("src/test/resources/data/proof_of_address/response_v1/complete.json"),
        type);

    String[] actualLines = prediction.getDocument().toString().split(System.lineSeparator());
    List<String> expectedLines = Files
        .readAllLines(Paths.get("src/test/resources/data/proof_of_address/response_v1/summary_full.rst"));
