...
public class CarteVitaleV1Test {
  ...
  void givenCarteVitaleV1_whenDeserialized_MustHaveAValidSummary() throws IOException {
    ...
    PredictResponse<CarteVitaleV1Inference> prediction = objectMapper.readValue(
      new File("src/test/resources/data/fr/carte_vitale/response_v1/complete.json"),
      type);

    String[] actualLines = prediction.getDocument().toString().split(System.lineSeparator());
    List<String> expectedLines = Files
      .readAllLines(Paths.get("src/test/resources/data/fr/carte_vitale/response_v1/summary_full.rst"));
